...
final class Privacy_Page {
	 ...
	 * The notice, rendered.
	 *
	 * In WordPress this class created and maintained a wp_post: it made a page,
	 * stamped it so it could recognise its own text later, and refused to
	 * overwrite anything a human had edited. That machinery existed because the
	 * notice had to live somewhere WordPress could serve it.
	 *
	 * Here it is a route -- /privacy -- rendered from this class on request. The
	 * stamp-and-do-not-overwrite dance has no purpose when there is no stored
	 * copy to overwrite, and the notice is now always current: it is generated
	 * from the retention period the system actually uses, every time, rather
	 * than from whatever that period happened to be on the day a page was
	 * created.
	 *
	 * The trade is real and worth stating. A church can no longer edit this
	 * through an admin screen; changing the wording means changing content()
	 * below. Given that every factual claim in it is derived from the code,
	 * text that cannot drift away from the code is the safer default for a
	 * document making legal promises about personal data.
	 */
	 ...
	 * Nothing to install.
	 *
	 * Kept because the installer calls it. Returns 0, which in the WordPress
	 * version meant "no page was created or changed" -- the honest answer here,
	 * since there is no page to create.
	 */
