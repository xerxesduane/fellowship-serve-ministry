...
final class App {
	...
	private static function is_app_screen(): bool {
		return isset( $_GET['page'] ) && Admin::PAGE_DASHBOARD === $_GET['page'];
	}

	public static function body_class( string $classes ): string {
		if ( self::is_app_screen() ) {
			$classes .= ' serve-app-fullscreen';
		}
	...
	public static function assets(): void {
		...
		wp_enqueue_style(
			'serve-app',
			SERVE_DASHBOARD_URL . 'admin/css/app.css',
			array(),
			SERVE_DASHBOARD_VERSION
		);

		// Config travels as a JSON island in the markup (see admin/views/app.php)
		// rather than inlined here, so the module stays a static cacheable file.
		wp_enqueue_script_module(
			'serve-app',
			SERVE_DASHBOARD_URL . 'admin/js/app.js',
			array(),
			SERVE_DASHBOARD_VERSION
		);
	}
